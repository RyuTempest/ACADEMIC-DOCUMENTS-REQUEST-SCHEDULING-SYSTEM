...
// Handle form submission for adding/updating slots
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	var_dump($_POST);
    $slot_date = $_POST['slot_date'];
    $start_time_am = isset($_POST['start_time_am']) ? $_POST['start_time_am'] : '08:00:00';
$end_time_am = isset($_POST['end_time_am']) ? $_POST['end_time_am'] : '12:00:00';
    $capacity_am = isset($_POST['capacity_am']) ? $_POST['capacity_am'] : 30;

	$start_time_pm = isset($_POST['start_time_pm']) ? $_POST['start_time_pm'] : '13:00:00';
	$end_time_pm = isset($_POST['end_time_pm']) ? $_POST['end_time_pm'] : '17:00:00';
    $capacity_pm = isset($_POST['capacity_pm']) ? $_POST['capacity_pm'] : 30;

	if (empty($slot_date) || empty($start_time_am) || empty($end_time_am) || empty($start_time_pm) || empty($end_time_pm)) {
		echo "All fields are required!";
		exit;
	}
	

    // Check if the slot for this date already exists
    $check_sql = "SELECT id FROM slots WHERE user_id = ? AND slot_date = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $update_sql = "UPDATE slots 
                       SET start_time_am = ?, end_time_am = ?, am_capacity = ?, 
                           start_time_pm = ?, end_time_pm = ?, pm_capacity = ? 
                       WHERE id = ?";
        $stmt_update = $conn->prepare($update_sql);
        if (!$stmt_update) {
            die("Prepare failed: " . $conn->error);
        }
        $stmt_update->bind_param("ssissii", $start_time_am, $end_time_am, $capacity_am, $start_time_pm, $end_time_pm, $capacity_pm, $slot_id);
        
        if ($stmt_update->execute()) {
            echo "Slot updated successfully!";
        } else {
            echo "Error updating slot: " . $stmt_update->error;
        }
        $stmt_update->close();
    } else {
        ...
        $insert_sql = "INSERT INTO slots (user_id, slot_date, start_time_am, end_time_am, am_capacity, am_booked, start_time_pm, end_time_pm, pm_capacity, pm_booked) 
                       VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, 0)";
        $stmt_insert = $conn->prepare($insert_sql);
        if (!$stmt_insert) {
            die("Prepare failed: " . $conn->error);
        }
        $stmt_insert->bind_param("isssissi", $user_id, $slot_date, $start_time_am, $end_time_am, $capacity_am, $start_time_pm, $end_time_pm, $capacity_pm);
        
        if ($stmt_insert->execute()) {
            echo "New slot inserted successfully!";
        } else {
            echo "Error inserting new slot: " . $stmt_insert->error;
        }
        $stmt_insert->close();
    }
		...
		<div class="sidebar" id="sidebar">
			<div class="sidebar-inner slimscroll">
				<div id="sidebar-menu" class="sidebar-menu">
					<ul>
						<li class="active"> <a href="/book/admin/dashboard.php"><i class="fas fa-tachometer-alt"></i> <span>Dashboard</span></a> </li>
                        ...
                        <li class="active"> <a href="/book/admin/calendar.php"><i class="fas fa-calendar-alt"></i> <span> Calendar </span></a></li>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="slotModalLabel">Add Slots</h5>
      ...
      <div class="modal-body">
        <form id="add_slot_form" method="post" action="">
          <div class="form-group">
            ...
            <input type="text" name="slot_date" id="slot_date" class="form-control datetimepicker" >
          </div>
          <div class="form-group">
            <label for="start_time_am">AM Start Time:</label>
            <input type="time" name="start_time_am" id="start_time_am" class="form-control" value="09:00" >
          </div>
          <div class="form-group">
            <label for="end_time_am">AM End Time:</label>
            <input type="time" name="end_time_am" id="end_time_am" class="form-control" value="12:00">
          </div>
          <div class="form-group">
            <label for="capacity_am">AM Slot Capacity (default: 30):</label>
            <input type="number" name="capacity_am" id="capacity_am" class="form-control" value="30" min="0">
          </div>
          <div class="form-group">
            <label for="start_time_pm">PM Start Time:</label>
            <input type="time" name="start_time_pm" id="start_time_pm" class="form-control" value="13:00">
          </div>
          <div class="form-group">
            <label for="end_time_pm">PM End Time:</label>
            <input type="time" name="end_time_pm" id="end_time_pm" class="form-control" value="17:00">
          </div>
          <div class="form-group">
            <label for="capacity_pm">PM Slot Capacity (default: 30):</label>
            <input type="number" name="capacity_pm" id="capacity_pm" class="form-control" value="30" min="0">
          </div>
...
$remaining_slots = isset($remaining_slots) && is_array($remaining_slots) ? $remaining_slots : [];

// Combine slots and remaining_slots arrays
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        events: '/book/admin/fetch.events.php', // Fetch events from the PHP file via AJAX
        validRange: {
